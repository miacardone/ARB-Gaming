  ...
  return (
    <>
      ...
      <p className="doc-page__re">
        RE: Case {c.id} · ARN {c.arn ?? '—'} · {c.networkLabel ?? brand.terms.claimProgram} {c.reasonCode}
      </p>
      ...
      <p>
        ...
        the cardholder&apos;s claim against our purchase, wallet-credit and game-session records and
        respectfully submit that the transaction was valid and the digital goods were delivered in full.
      </p>
      ...
      <table className="doc-page__table">
        <tbody>
          ...
          <tr><td>{titleCase(brand.terms.order)} reference</td><td>{c.orderId}</td></tr>
          <tr><td>{titleCase(brand.terms.item)}</td><td>{c.itemTitle}</td></tr>
          <tr><td>{brand.terms.deliveryLabel}</td><td>{c.carrier} · {c.tracking}</td></tr>
        </tbody>
      </table>

      <p>
        The purchase was applied to the account balance by {c.carrier.toLowerCase()} against reference{' '}
        {c.tracking}, and the wallet ledger records the credit state as{' '}
        <strong>{c.itemCondition.toLowerCase()}</strong>. The session log shows the balance was subsequently
        played on &ldquo;{c.itemTitle}&rdquo; ({c.itemCategory}) from a device previously registered to this
        account. The purchase confirmation, the credit ledger and the accepted terms of service are enclosed
        with this response.
      </p>
...
function SalesReceipt({ c, brand }) {
  return (
    <>
      ...
      <p className="doc-page__re">{titleCase(brand.terms.order)} {c.orderId}</p>

      <table className="doc-page__table">
        <tbody>
          <tr><td>{titleCase(brand.terms.order)} placed</td><td>{formatDate(c.orderPlacedAt)}</td></tr>
      ...
      <table className="doc-page__items">
        ...
        <tbody>
          <tr>
            <td>{c.itemTitle}<br /><span className="doc-page__meta" style={{ textAlign: 'left' }}>{brand.terms.conditionLabel}: {c.itemCondition}</span></td>
            <td>{formatCurrency(c.itemPrice, c.currency)}</td>
          </tr>
          <tr><td>{brand.terms.feeLabel}</td><td>{formatCurrency(c.shipping, c.currency)}</td></tr>
        </tbody>
        <tfoot>
      ...
      <p className="doc-page__foot-note">
        Payment captured in full · No refund has been issued against this {brand.terms.order}.<br />
...
function GenericDoc({ c, brand, title }) {
  return (
    <>
      ...
      <table className="doc-page__table">
        <tbody>
          <tr><td>{titleCase(brand.terms.order)}</td><td>{c.orderId}</td></tr>
          <tr><td>{titleCase(brand.terms.item)}</td><td>{c.itemTitle}</td></tr>
          <tr><td>{titleCase(brand.terms.buyer)}</td><td>{c.buyer}</td></tr>
          <tr><td>{titleCase(brand.terms.seller)}</td><td>{c.seller}</td></tr>
          <tr><td>{brand.terms.deliveryLabel}</td><td>{c.carrier}</td></tr>
          <tr><td>{brand.terms.deliveryRefLabel}</td><td>{c.tracking}</td></tr>
