...
const snapTo99 = (v) => Math.max(0.99, Math.round(v) - 0.01);
 ...
 *  `wrong_item` is the post-restriction claim reason — it is a compliance
 *  matter, not a payments one, so it routes to the compliance queue rather than
 *  sitting in the general claim pool. */
function queueFor(caseType, reasonCode, amount, cycleId) {
  if (caseType === 'claim') {
    ...
    if (reasonCode === 'wrong_item') return 'logistics';
...
function generate() {
  ...
  const cases = Array.from({ length: COUNT }, (_, index) => {
    ...
    // Coin packages sell at fixed .99 price points, so a drawn price is snapped
    // onto that ladder. Without this the book reads as generated — nobody has
    // ever bought a $9.84 package.
    const itemPrice = snapTo99(draw.money(itemSpec.low, itemSpec.high));
    const shipping = draw.money(0.3, 2.4);
    ...
    return {
      ...
      orderId: `${brand.numbering.orderPrefix}-${draw.digits(8)}`,
