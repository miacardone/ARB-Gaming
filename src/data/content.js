...
export const API_BASE = `https://api.${brand.emailDomain}/v1`;
...
export const API_ENDPOINTS = [
  {
    ...
    description: `One collection holds chargebacks and ${brand.terms.claimProgram} claims, distinguished by \`caseType\`. Card fields are null on claims; ${brand.terms.marketplace} fields are present on both.`,
    query: [
    ...
    response: {
      data: [{ id: `${brand.numbering.prefix}-620008`, caseType: 'chargeback', status: 'working', disputeAmount: 199.99, currency: brand.currency, network: 'visa', reasonCode: '13.1', cycleId: 'first_cb', queueId: 'not_received', arn: '74537286104920117364520', orderId: `${brand.numbering.orderPrefix}-38340681`, seller: '@crownfall_studios_1' }],
  ...
  },
  {
    ...
    description: `Includes the ${brand.terms.marketplace} context a chargeback carries — ${brand.terms.item}, ${brand.terms.order}, ${brand.terms.seller} and the coin-credit state — which is what makes a 13.1 defensible without opening a second system.`,
    params: [{ name: 'caseId', type: 'string', required: true, description: `e.g. ${brand.numbering.prefix}-620008` }],
    response: { id: `${brand.numbering.prefix}-620008`, caseType: 'chargeback', status: 'working', itemTitle: 'Ironwood Fortune Grand', documents: [{ id: `${brand.numbering.prefix}-620008-merchant-1`, title: 'Representment Letter' }], flags: ['consolidated', 'high_value'] },
  ...
  },
  {
    ...
    response: { id: `${brand.numbering.prefix}-620008`, status: 'pended' },
  ...
  },
  {
    ...
    response: { id: `${brand.numbering.prefix}-620008`, status: 'represented', outcome: 'pending' },
  ...
  },
  {
    ...
    params: [{ name: 'caseId', type: 'string', required: true }],
    response: { groups: [{ id: `same_order:${brand.numbering.orderPrefix}-48030237`, ruleId: 'same_order', size: 2, totalExposure: 599.98, crossChannel: true, duplicateRefundRisk: true, caseIds: [`${brand.numbering.prefix}-620872`, `${brand.numbering.prefix}-620078`] }] },
    errors: [{ code: 404, meaning: 'No case with that id.' }],
  },
  {
  ...
  },
  {
    ...
    response: { matched: 46, total: 1200, sample: [`${brand.numbering.prefix}-620012`] },
  ...
  },
  {
    ...
    response: { data: [{ id: 'not_received', label: brand.queues.find((q) => q.id === 'not_received').label, sla: 48, casesInQueue: 143 }] },
    errors: [{ code: 401, meaning: 'Missing or expired bearer token.' }],
  },
  {
  ...
  },
  {
    ...
    response: { data: [{ id: 'u5', name: USERS[4].name, role: brand.terms.analyst, skills: ['All Dispute Response'] }] },
  ...
  },
  {
    ...
    response: { numbering: { prefix: brand.numbering.prefix, digits: 6 }, dueDateOffsets: { schemeDays: { visa: 30, mastercard: 45, amex: 20 }, internalBufferDays: 4 }, thresholds: { riskAmount: brand.thresholds.riskAmount } },
...
export const SAVED_REPORTS = [
  ...
  { id: 'rep4', name: `${titleCase(brand.terms.seller)} dispute-cluster review`, type: MARKETPLACE_TYPE, templateId: 'tpl_marketplace', dateCreated: ago(18), createdBy: USERS[6].email, rowCount: 64, fileSize: '21 KB', format: 'CSV', schedule: { mode: 'on_demand' } },
...
export const HELP_VIDEOS = [
  ...
  { id: 'v2', title: 'Understanding consolidation', description: `Why cases get linked, and what to do when one ${brand.terms.order} is disputed twice.`, duration: '4:48', level: 'Getting started' },
...
export const HELP_DOCS = [
  ...
  { id: 'd3', title: 'Evidence that actually wins', description: 'What issuers accept on digital goods: wallet credit ledgers, round-level session logs, device and IP history.', category: 'Playbook', readingMinutes: 11 },
  ...
  { id: 'd9', title: 'Responsible gaming and the payments queue', description: 'When a charge is a compliance matter first and a chargeback second.', category: 'Playbook', readingMinutes: 6 },
...
export const HELP_FAQ = [
  {
    ...
    answer: `Because you usually cannot defend a card dispute without them. A “services not received” reason code on a coin purchase is an argument about whether the balance was credited and played, so the ${brand.terms.item}, the credit state and the ${brand.terms.seller}’s session logs sit on the case alongside the ARN. Both intake paths share one record shape for exactly this reason.`,
  },
  {
    id: 'f2',
...
export const HELP_CONTACT_TOPICS = ['Access or permissions', 'Case data looks wrong', 'Rules and automation', 'API or webhooks', 'Responsible gaming escalation', 'Billing and contract', 'Something else'];
