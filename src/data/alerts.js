 ...
 *
 * ARB is single-tenant, unlike a multi-client alerts platform, so the
 * "client" rollup dimension here is the tenant's own three consumer brands
 * rather than an invented client roster.
 */
 ...
 *  to a brand. Digital-goods descriptors are short, upper-case and often
 *  abbreviated by the issuer, which is exactly why the match can fail and why
 *  this table needs a maintenance screen at all. */
const IDENTIFIER_SEED = [
  { entityId: brand.entities[0].id, mid: brand.entities[0].mid, identifier: 'MODO*CASINO' },
  { entityId: brand.entities[0].id, mid: brand.entities[0].mid, identifier: 'MODO GOLD COINS' },
  { entityId: brand.entities[0].id, mid: brand.entities[0].mid, identifier: 'MODO.US*COINS' },
  { entityId: brand.entities[1].id, mid: brand.entities[1].mid, identifier: 'MODO LIVE STUDIO' },
  { entityId: brand.entities[1].id, mid: brand.entities[1].mid, identifier: 'MODO*LIVEDEALER' },
  { entityId: brand.entities[2].id, mid: brand.entities[2].mid, identifier: 'PCHGAMES.COM' },
  { entityId: brand.entities[2].id, mid: brand.entities[2].mid, identifier: 'PCH*TOKENS' },
  { entityId: brand.entities[2].id, mid: brand.entities[2].mid, identifier: 'PCH SUPERPRIZE' },
];
...
const roleFor = (u) => {
  ...
  if (['Weekend Cover', 'Account Integrity'].includes(u.group)) return 'No access';
  return 'Alerts User';
};
 ...
 * sees matches what ARB has on file. Informational, not actionable:
