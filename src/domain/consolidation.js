 ...
 * literally — correct for a marketplace tenant, wrong the moment a tenant's
 * vocabulary diverges (ARB's `seller` term is "game studio"). Both now
...
export function explainGroup(group) {
  if (group.duplicateRefundRisk) {
    return `This ${brand.terms.order} is being disputed through two channels at once — a card ${brand.terms.chargeback} and a ${brand.terms.claimProgram} ${brand.terms.claim}. Worked separately, the same ${brand.terms.order} gets refunded twice.`;
  }
  switch (group.ruleId) {
    case 'same_card':
